<script lang="ts">
    ...
    import PresentationListing from "./PresentationListing.svelte";

    export let topic: Topic;
</script>

<section id={topic.name} class="min-h-screen w-screen border-t-8 pt-8" style:border-color={topic.color}>
    <div class="container mx-auto flex flex-col md:flex-row">
        <div class="flex flex-col align-center items-center px-10">
            <img src="/{topic.name.toLowerCase()}.svg" alt="Cover" class="w-64 h-64 pb-4">
            <h1 class="text-4xl font-bold" style:color={topic.color}>{topic.name}</h1>
            <p class="text-gray-600">{topic.description}</p>
        </div>
        <div class="flex flex-col">
            {#each topic.presentations as presentation}
                <div class="wrapper">
                    <PresentationListing {presentation}/>
                </div>
            {:else}
                <div class="flex flex-row">
                    <svg class="h-64" style:color={topic.color} xmlns="http://www.w3.org/2000/svg" data-name="Layer 1"
                        ...
                        <path d="M398.86279,261.73389a9.0157,9.0157,0,0,1-8.61133-6.3667l-12.88037-42.07178a8.99884,8.99884,0,0,1,5.9712-11.24023l175.939-53.86377a9.00867,9.00867,0,0,1,11.24072,5.9707l12.88037,42.07227a9.01029,9.01029,0,0,1-5.9707,11.24072L401.49219,261.33887A8.976,8.976,0,0,1,398.86279,261.73389Z"
                              transform="translate(-276.18187 -133.91309)" fill="currentColor"/>
                        <circle cx="190.15351" cy="24.95465" r="20" fill="currentColor"/>
                        ...
                        <path d="M801.81836,318.08691h-184a9.01015,9.01015,0,0,1-9-9v-44a9.01016,9.01016,0,0,1,9-9h184a9.01016,9.01016,0,0,1,9,9v44A9.01015,9.01015,0,0,1,801.81836,318.08691Z"
                              transform="translate(-276.18187 -133.91309)" fill="currentColor"/>
                        <circle cx="433.63626" cy="105.17383" r="20" fill="currentColor"/>
                        <circle cx="433.63626" cy="105.17383" r="12.18187" fill="#fff"/>
                    </svg>
                    <section>
                        <h1 class="text-2xl mb-4">Sorry...</h1>
                        <p>
                            There's currently no presentations available for the topic
